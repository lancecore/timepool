...
TimePool runs on almost any PHP host (shared hosting, cPanel, a VPS, even a Raspberry Pi). No database server or terminal is required — it uses a self-contained SQLite file.
...
2. **Upload the files.** Using your host's File Manager or any FTP app, upload the entire TimePool folder's contents into that document root. (If you only have `install.php`, it can fetch the rest when `RELEASE_URL` is configured — otherwise upload the full package.)
...
Either works with no configuration changes — TimePool detects where it's installed and builds links accordingly.
